...
;(function ($, window, document, undefined) {
    ...
    $.scrollClass = function (el, options) {
        ...
        base.onScroll = function () {
			...
			//if (base.almostInViewport()) {
			//} else 
			if (base.inViewport()) {
				//console.log('inViewport');
                base.$el.addClass(dataAttr);
                base.$el.addClass('in-view');
                // Callback
				...
				//console.log('else inViewport');
                //base.$el.removeClass(dataAttr);
                base.$el.removeClass('in-view');
                viewed = false;
			}
		...
		// If element visible in viewport
        base.almostInViewport = function () {
            var elRect = base.el.getBoundingClientRect(),
                thresholdNum = (base.options.preThreshold / 100) * elRect.height;
				console.info(elRect);
				console.info("thresholdNum", thresholdNum);
			return false;
            return (
                elRect.top + thresholdNum <= base.$win.height() && elRect.bottom - thresholdNum >= 0 + base.options.offsetTop
            );
